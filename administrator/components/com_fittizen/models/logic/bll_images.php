<?php
/**
 * Logic layer object of D.B. table fittizen_images this object
 * has the basic CRUD functions build-in, for
 * normalized databases tables.
 *
 * @author Gabriel Gonzalez Disla
 */
class bll_images extends fittizen_images
{
   /**
    ...
    */
     ...
     *
     * @return boolean|bll_images Not false on success.
     */
     ...
     *
     * @return bll_images not false on success.
     */
     ...
     * @param int $fitinfo_id id of the image to unset as main
     *
     * @return boolean not false on success.
     */
    ...
    /**
     * Gets all the images from a fitinfo profile
     * 
     ...
     * @return bll_images dbobject or false on failure.
     */
    public static function getImages($fitinfo_id, $DESC=true,
            $order_field='created_date',
            $lower_limit=null, $upper_limit=null)
    {
        $image = new bll_images(-1);
        return $image->findAll('fitinfo_id', $fitinfo_id,
                $DESC, $order_field, $lower_limit, $upper_limit);
    }
    
    /**
     * Gets all the images from an album
     * 
     ...
     * @return bll_images dbobject or false on failure.
     */
    public static function getAlbumImages($album_id, $DESC=true,
            $order_field='created_date',
            $lower_limit=null, $upper_limit=null)
    {
        $image = new bll_images(-1);
        return $image->findAll('album_id', $album_id,
                $DESC, $order_field, $lower_limit, $upper_limit);
    }
    
    /**
     * Gets all the comments from a image
     * @param  boolean $DESC ascendent
     * @param  string  $order_field Field for the order by
     * @param  integer $lower_limit  lower limit on the query, it must be
     * an integer otherwise is going to be ignored
     * @param  integer $upper_limit higher limit on the query, it must be
     * an integer otherwise is going to be ignored
     * @return fittizen_comment_image array of comments
     */
    public function get_comments($DESC=true, $order_field='created_date',
            $lower_limit=null, $upper_limit=null)
    {
        return bll_fitinfos::get_comments(bll_fitinfos_constants::IMAGE,
        bll_fitinfos_constants::IMAGE_ID, $this->id,
        $DESC, $order_field, $lower_limit, $upper_limit);
    }
    
    /**
     * Gets all the tags from a image
     * @param  boolean $DESC ascendent
     * @param  string  $order_field Field for the order by
     * @param  integer $lower_limit  lower limit on the query, it must be
     * an integer otherwise is going to be ignored
     * @param  integer $upper_limit higher limit on the query, it must be
     * an integer otherwise is going to be ignored
     * @return fittizen_images_tags array of comments
     */
    public function get_tags($DESC=true, $order_field='created_date',
            $lower_limit=null, $upper_limit=null)
    {
        return bll_fitinfos::get_tags(bll_fitinfos_constants::IMAGES, 
                bll_fitinfos_constants::IMAGE_ID, $this->id,
        $DESC, $order_field, $lower_limit, $upper_limit );
    }
    
    /**
     * Remove a tag of a image
    ...
    {
        return bll_fitinfos::remove_tag(bll_fitinfos_constants::IMAGES, $id);
    }
    
    /**
     * Remove a comment of a image
     * @param int $id id of the comment to remove
     * 
     * @return boolean|dbobject Not false on success.
     */
    ...
    {
        return bll_fitinfos::remove_comment(bll_fitinfos_constants::IMAGE, $id);
    }
    
    /**
     * Get a comment from the image
     * @param int $id id of the comment to get
     * 
     * @return fittizen_comment_image the comment
     */
    public function get_comment($id)
    {
        return bll_fitinfos::get_comment(bll_fitinfos_constants::IMAGE, $id);
    }
     ...
     * 
     * @return fittizen_comment_images_mentions array of mentions
     */
    public function get_comment_mentions($comment_id)
    {
        return bll_fitinfos::get_mentions(bll_fitinfos_constants::COMMENT_IMAGES, 
                bll_fitinfos_constants::IMAGE_ID, $comment_id);
    }
